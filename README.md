...
4. 등록 구조가 같은 기존 버전을 업데이트할 때는 현재 한결 프로세스와 입력 메뉴 연결을 유지합니다. 입력과 `한결 설정...`은 계속 사용할 수 있으며, 새 실행 파일은 다음 로그인부터 적용됩니다.

한결 앱 번들은 기본적으로 `/Library/Input Methods/Hangyeol.app`에 설치됩니다. 3.0 설치기는 2.x 설정을 한 번 이전하고 기존 앱·입력 소스 등록·설치 영수증을 정리합니다. 패키지의 등록 메타데이터와 설치본의 bundle ID·IMK connection·input mode schema가 같으면 일반 업데이트로 분류해 현재 IMK와 입력 메뉴 연결을 건드리지 않습니다. 최초 설치나 등록 구조 변경에서는 서명된 도우미가 현재 선택을 TIS에서 확인하고, 필요하면 안전한 영문 입력 소스로 이탈한 뒤 한결 소유 프로세스만 종료합니다. `ABC 끄기`를 사용 중이면 설치 중에만 ABC를 임시 활성화하고 새 한결 선택이 확인된 뒤 다시 끕니다. 등록·parent 활성화·mode 활성화·선택 복원은 각각 다른 프로세스의 확인을 통과해야 다음 단계로 진행합니다. macOS 입력기 관련 시스템 프로세스와 다른 앱은 강제 재시작하지 않습니다.
...
- **업데이트 직후 새 버전이 적용되지 않을 때**
  일반 업데이트는 현재 앱들의 입력 연결과 `한결 설정...` 메뉴를 보호하기 위해 실행 중인 한결을 유지합니다. 설치 직후에는 기존 실행본이 계속 동작하고, 로그아웃 후 로그인하거나 Mac을 재시동하면 새 실행 파일이 적용됩니다. 최초 설치나 등록 구조 변경에서 활성화가 끝나지 않은 경우에도 다음 로그인의 1회성 복구 작업이 다시 시도합니다.
