...
한결 앱 번들은 기본적으로 `/Library/Input Methods/Hangyeol.app`에 설치됩니다. 3.0 설치기는 2.x 설정을 한 번 이전하고 기존 앱·입력 소스 등록·설치 영수증을 정리합니다. 3.0.23부터는 일반 업데이트도 서명된 도우미가 현재 선택을 TIS에서 확인하고, 필요하면 안전한 영문 입력 소스로 이탈한 뒤 한결 소유 프로세스를 종료합니다. 파일 교체 후 새 한결을 현재 로그인 세션에서 실행하고 활성화 완료를 기다립니다. `ABC 끄기`를 사용 중이면 설치 중에만 ABC를 임시 활성화하고 새 한결 선택과 준비 상태가 확인된 뒤 다시 끕니다. 등록·parent 활성화·mode 활성화·선택 복원은 각각 다른 프로세스의 확인을 통과해야 다음 단계로 진행합니다. 실패하면 임시 입력 소스와 다음 로그인용 복구 작업을 유지합니다. macOS 입력기 관련 시스템 프로세스와 다른 앱은 강제 재시작하지 않습니다. 재로그인 없는 메뉴·입력 복구는 실제 설치 후 별도로 검증해야 하며, 패키지 생성 성공만으로 보장하지 않습니다.
