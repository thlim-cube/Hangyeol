...
4. 기존 버전을 업데이트할 때는 현재 입력 연결과 입력 소스 등록을 유지합니다. 설치 중이던 앱은 계속 사용할 수 있고, 새 버전 실행 파일은 로그아웃 후 로그인하거나 재시동하면 적용됩니다.

한결 앱 번들은 기본적으로 `/Library/Input Methods/Hangyeol.app`에 설치됩니다. 3.0 설치기는 2.x 설정을 한 번 이전하고 기존 앱·입력 소스 등록·설치 영수증을 정리합니다. 최초 설치에만 현재 로그인 사용자의 macOS 표준 TIS API를 사용하고, 활성화 요청을 보낸 프로세스의 캐시는 성공 근거로 사용하지 않습니다. 이후 3.x 업데이트는 PackageKit의 원자적 교체를 사용하되 실행 중인 IMK 서버와 TIS 등록을 그대로 유지합니다. 입력기 관련 시스템 프로세스를 강제 재시작하지 않으며, 다른 입력 소스와 ABC도 자동으로 삭제하지 않습니다.
...
- **입력 소스가 중복으로 보일 때**
  최초 설치에서는 현재 사용자의 한결 stale 항목을 정리하고 TIS API로 현재 번들을 등록합니다. 일반 업데이트는 정상 등록을 다시 쓰지 않습니다. 설치 후에도 중복이 남으면 macOS 입력 소스 설정에서 표시된 항목과 한결 버전을 확인해 이슈에 첨부해 주세요.

- **업데이트 직후 새 버전이 적용되지 않을 때**
  실행 중인 앱의 IMK 연결과 입력 소스 등록을 끊지 않는 업데이트 정책입니다. 로그아웃 후 다시 로그인하거나 Mac을 재시동하면 새 실행 파일이 적용됩니다. 설치 직후 적용을 위해 입력기나 시스템 agent를 강제 종료하지 않습니다.
