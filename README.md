...
4. 등록 구조가 같은 기존 버전을 업데이트할 때는 현재 한결 프로세스를 종료하지 않고, 다음 로그인에 새 번들을 TIS에 다시 등록합니다. 새 실행 파일은 다음 로그인부터 적용됩니다. 설치 직후 메뉴 표시 문제는 아래 문제 해결 항목을 참고해 주세요.

한결 앱 번들은 기본적으로 `/Library/Input Methods/Hangyeol.app`에 설치됩니다. 3.0 설치기는 2.x 설정을 한 번 이전하고 기존 앱·입력 소스 등록·설치 영수증을 정리합니다. 패키지의 등록 메타데이터와 설치본의 bundle ID·IMK connection·input mode schema가 같으면 일반 업데이트로 분류해 현재 IMK와 입력 메뉴 연결을 건드리지 않고 다음 로그인용 등록 복구 작업을 남깁니다. 최초 설치나 등록 구조 변경에서는 서명된 도우미가 현재 선택을 TIS에서 확인하고, 필요하면 안전한 영문 입력 소스로 이탈한 뒤 한결 소유 프로세스만 종료합니다. `ABC 끄기`를 사용 중이면 설치 중에만 ABC를 임시 활성화하고 새 한결 선택이 확인된 뒤 다시 끕니다. 등록·parent 활성화·mode 활성화·선택 복원은 각각 다른 프로세스의 확인을 통과해야 다음 단계로 진행합니다. macOS 입력기 관련 시스템 프로세스와 다른 앱은 강제 재시작하지 않습니다.
...
한결 설정의 자판 배열 영역에서 추가 모음 결합과 분해 삭제를 하나의 옵션으로 선택할 수 있습니다. 기본값은 **꺼짐**입니다.

- 꺼짐: `ㅕ + ㅣ`는 `ㅕㅣ`로 입력됩니다. 직접 입력한 `ㅐ`나 `ㅖ`를 Backspace로 지우면 `ㅏ`나 `ㅕ`를 남기지 않습니다. 예를 들어 `계`의 모음을 지우면 `ㄱ`이 남습니다.
- 켜짐: `ㅏ·ㅑ·ㅓ·ㅕ + ㅣ → ㅐ·ㅒ·ㅔ·ㅖ` 결합과 역방향 분해 삭제를 허용합니다.

일반적인 `ㅗ + ㅏ → ㅘ` 조합과 `와 → 오`, `맑 → 말` 삭제는 이 옵션과 무관하게 유지합니다. 설정 변경은 다음 입력부터 적용됩니다.
...
| 추가 모음 규칙 | 모음 결합·분해 삭제를 함께 선택 가능 (기본 꺼짐) |
...
- **설치 직후 설정 메뉴가 사라질 때**
  입력 소스가 등록·활성 상태여도 실제 메뉴가 표시된다는 보장은 없습니다. 이 현상은 아직 실호스트 원인 확인이 필요합니다. 설치한 버전, 실행 중인 버전, 설치 직후와 재로그인 후의 메뉴 상태를 구분해서 확인해 주세요. 현재 로컬 패키지의 단위 테스트·서명 검증만으로 이 문제가 해결됐다고 판단하지 않습니다.
...
- **업데이트 직후 새 버전이 적용되지 않을 때**
  일반 업데이트는 현재 앱들의 입력 연결과 `한결 설정...` 메뉴를 보호하기 위해 실행 중인 한결을 유지하고 다음 로그인용 등록 복구 작업을 예약합니다. 설치 직후에는 기존 실행본이 계속 동작하고, 로그아웃 후 로그인하거나 Mac을 재시동하면 새 실행 파일이 등록·적용됩니다. 최초 설치나 등록 구조 변경에서 활성화가 끝나지 않은 경우에도 다음 로그인의 1회성 복구 작업이 다시 시도합니다.
