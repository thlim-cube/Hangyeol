...
- 정식 앱과 새 앱의 입력기 연결 이름이 다르면 현재 세션의 입력기를 교체하지 않고 로그아웃 이후 적용으로 넘깁니다. 3.1.14 실제 설치에서 한결 항목만 남고 한영 전환·설정 메뉴 연결이 끊긴 경로를 차단합니다. 같은 연결 이름의 업데이트는 기존 즉시 적용 경로를 유지합니다.

### Verification pending

- 3.1.15 패키지의 실제 관리자 설치 및 로그아웃 후 적용은 별도 확인이 필요합니다. 현재 호스트는 정식 3.1.12를 사용합니다.

## [3.1.14] - 2026-09-23

### Fixed

- 입력기 교체 후 기존 Slack에서 키 입력이 막히거나 초점을 잃는 문제를 수정했습니다. IMK 연결 이름을 번들 ID 기반 이름으로 통일하고, 시스템 재연결 요청이 설정 창을 자동으로 띄우지 않도록 했습니다. 설정은 입력 메뉴에서 계속 열 수 있습니다.
- 현재 세션 교체 시 강제 종료 신호 대신 앱 정상 종료를 요청하고, 새 앱의 초기화 완료를 확인한 뒤 입력 소스를 복귀시킵니다. 기존 연결 이름에서 새 이름으로의 변경만 서명·등록 검증 아래 허용합니다.

### Verified

- 기존 Slack 프로세스를 유지하고 백그라운드에서 임시 앱을 교체한 뒤 첫 한글 입력, 한영 전환, 시스템 재연결 시 초점 유지를 검증했습니다. 수신인 없는 빈 초안에서 검사했으며 메시지는 전송하지 않았습니다.
- 단위 테스트 599개, HangyeolVerify, Slack 및 TextEdit·Chrome 입력 검증을 통과했습니다.

### Verification pending

- 3.1.14 패키지의 실제 관리자 설치와 이후 재로그인 적용은 별도 확인 대상입니다. 현재 세션은 패키지에서 꺼낸 서명 앱과 도우미로 검증했습니다.
...
- 설치 전부터 실행 중인 Slack에서는 붙여넣기·ABC 입력만 되고 한결 키 입력이 막혔습니다. Slack 재실행 후 복구됐으며 3.1.14에서 기존 앱 재연결과 초점 유지 검증을 추가했습니다.
