...
- 로컬 설치 완료 시 정식 입력기 번들을 유지하면서 서명된 새 앱의 임시 복사본으로 현재 로그인 세션을 전환합니다. 별도 도우미가 기존 조합 종료, 이전 프로세스 종료, 시스템 앱 실행, 입력 소스 복귀 순서로 처리하며 실패하면 정식 앱 복구를 시도합니다.
- 임시 앱은 설치 당시 사용자·로그인 세션에서만 실행됩니다. 다른 로그인에서 임시 앱이 다시 호출되면 IMK 서버를 만들기 전에 정식 앱을 실행합니다.
- 새 버전의 정식 경로 교체는 기존과 같이 로그아웃 이후로 미룹니다. 임시 앱 CLI를 통한 입력 소스 재등록·복구 예약은 실행하지 않습니다.

### Verified

- 실제 패키지 설치에서 정식 앱 3.1.12/128을 유지하고 임시 앱 3.1.13/129 하나가 실행되는 것을 확인했습니다. 사용자가 설치 후 한결 표시를 확인했고, 설치본이 실행한 앱에서 TextEdit·Chrome 한글 조합과 기존 영문 모드 복원 검사 3개가 통과했습니다.
- 전체 단위 테스트 598개와 패키지 내용·서명 검증을 통과했습니다. 세션 만료를 모의한 실행에서 정식 앱 하나로 복귀하는 것을 확인했습니다.

### Verification pending

- 이번 임시 적용 뒤 실제 로그아웃·재로그인 경로와 설치 직후 설정·정보 메뉴의 상세 표시는 아직 확인하지 않았습니다.

## [3.1.12] - 2026-09-23

### Changed

- 로컬 설치 패키지에서 현재 로그인 중 실행하던 preinstall·postinstall 스크립트와 임시 한결 실행을 제거했습니다. 패키지는 다음 세션에 적용할 서명 앱 압축 파일과 시스템 갱신 작업만 설치합니다.
- 시스템 갱신 작업이 로그인 완료 후 시작되면 종료하지 않고 안전한 로그아웃 상태를 기다리도록 변경했습니다.

### Verified

- 실제 설치 직후 기존 한결 프로세스와 메뉴가 유지됐고, 시스템 갱신 작업을 시작한 뒤 로그아웃·재로그인하여 정식 앱 3.1.12/128 적용을 확인했습니다. 사용자가 입력 소스 재등록 없이 설정·정보 메뉴, 한글 입력과 한영 전환이 모두 정상임을 확인했습니다.
- 이번 검증은 이미 등록된 시스템 갱신 작업을 수동 시작한 경로입니다. 새 시스템 작업의 재부팅 자동 시작과 현재 세션에서 새 버전 즉시 적용은 별도 검증 대상입니다.

## [3.1.11] - 2026-09-23

### Changed

- 로컬 업데이트 패키지는 서명된 새 앱을 입력기 폴더 밖의 압축 파일로 설치합니다. 현재 로그인 세션의 정식 한결 번들은 건드리지 않고, 다음 재부팅에 로그인 전 시스템 작업이 서명과 체크섬을 확인한 뒤 교체합니다.
- 새 버전의 입력 소스 복구 요청은 다음 로그인용으로만 예약합니다.

### Known issue

- 실제 설치 직후 한결 메뉴가 사라지고 재부팅 후 복구됐습니다. 설치 로그에는 임시 한결 실행 파일과 사용자 복구 에이전트가 실행·갱신된 뒤 입력 메뉴 재구성이 나타납니다. 어느 동작이 원인인지는 아직 확정되지 않았습니다.
- 자동 로그인 뒤 시스템 갱신 작업이 실행되어 정식 앱 교체를 연기했습니다. 재부팅 후에도 정식 앱은 3.1.10이었습니다.

## [3.1.10] - 2026-09-23

### Changed

- 일반 업데이트에서 실행 중인 한결 입력기를 종료하거나 임시 앱으로 넘기지 않습니다. 설치 전에는 입력 소스 상태만 기록하고, 설치 후에는 현재 세션의 입력기와 메뉴를 유지한 채 다음 로그인에 정식 앱을 확인하는 복구 요청만 예약합니다.

### Known issue

- 실제 일반 업데이트에서 기존 한결 프로세스가 살아 있었지만 입력 메뉴에서 한결과 설정·정보가 사라졌습니다. 실행 중인 프로세스를 보존하는 것만으로는 정식 입력기 번들 갱신에 따른 등록 변화를 막지 못했습니다.
...
- 실제 설치에서 한결과 설정·정보 메뉴가 사라졌습니다. TIS 재등록·활성화·선택과 정식 앱 실행도 메뉴를 복구하지 못했으며, 시스템 설정에서 입력 소스를 수동 제거·재추가한 뒤 복구됐습니다.
...
- 같은 입력란에서 문서 접근 가능 여부만 일시적으로 달라질 때, 정확히 확인되는 한글 조합까지 버리던 경로를 수정했습니다. 입력 방식과 실제 조합 내용 검증은 유지합니다.
- 한영 전환·포커스 이동으로 조합을 확정하거나 버린 뒤에도 adapter에 남던 옛 조합 위치를 정리합니다. 다음 중간 삽입과 Forward Delete가 이전 위치를 물려받지 않습니다.
- 조합 확정이나 콜론 입력 뒤 불필요한 빈 조합 취소를 보내지 않습니다. 마지막 자모의 Backspace와 Escape는 실제 남아 있는 조합을 계속 취소합니다.
- IMK 범위 조회 중 입력란이 바뀌면 조회 전에 얻은 쓰기 권한을 재검증해 다른 필드에 조합을 쓰지 않습니다.

### Tests

- 위 경계의 수정 전 실패를 확인하고 회귀 테스트를 추가했습니다. Chrome 실입력 러너에 중간 삽입·Backspace·한영 전환과 `:완료:`의 정확한 Unicode 검증을 추가했습니다. 로컬 contenteditable 검증과 실제 Slack의 이모티콘 자동완성 검증은 구분합니다.

## [3.0.20] - 2026-09-10

### Fixed

- 창을 다녀온 뒤 Codex 등 Blink 편집기에 조합 표시만 남으면 Forward Delete가 다음 문자 대신 조합 중이던 문자를 지우던 경로를 수정했습니다. 내부 조합이 비어 있어도 현재 비보안 입력란에서 읽고 검증한 조합을 먼저 확정한 뒤 다음 문자 하나를 지웁니다. 옛 입력란의 조합·쓰기 권한은 복원하지 않습니다.
- `가나다라 → 가나마다라 → 창 전환·복귀 → 가나마라` 회귀와 보안·필드 변경·읽기 실패·수정키 경계 테스트를 추가했습니다. 실제 Codex 설치본에서의 재현 확인은 별도 검증 대상입니다.

## [3.0.19] - 2026-09-08

### Added

- 추가 모음 결합과 분해 삭제를 하나의 기본 꺼짐 옵션으로 분리합니다. 켜면 `ㅕ + ㅣ → ㅖ`, `ㅖ → ㅕ` 등을 허용하고, 끄면 추가 결합과 분해를 하지 않습니다. 일반 모음 조합과 자음 삭제는 유지합니다.

### Fixed

- 한영 전환의 키보드 레이아웃 변경 중 입력이 재진입하면 첫 글자가 이전 언어로 처리되던 순서 문제를 수정했습니다. 조합을 확정하고 새 내부 모드를 게시한 뒤 호스트 레이아웃을 변경합니다.
- 전환키나 한자키 suppression 상태에서 합성 단축키 보존 판단보다 물리 modifier 재구성이 먼저 적용되어 Command·Shift가 사라지던 분기를 수정했습니다.
- 지연 Delete·Return을 예약 시점의 입력란·어댑터·쓰기 승인에 묶었습니다. 다른 필드나 Secure Input을 거쳐 다시 입력이 허용되더라도 이전 예약은 실행되지 않습니다.

### Known Issues

- 설치 직후 입력 소스 설정 메뉴가 사라지는 현상은 미해결입니다. 입력 소스 등록 상태, 단위 테스트, 패키지 검증은 실제 메뉴 표시의 증거를 대신하지 않습니다.
