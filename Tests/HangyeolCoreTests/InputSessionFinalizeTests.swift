...
struct InputSessionFinalizeTests {
    ...
    func compositionRootUsesLiveDirectInsertionPreference() {
        ...
        let session = HangyeolInputController.makeInputSession(
            client: client,
            context: context(bundleId: client.bundleID, documentAccessSafe: true),
            composer: composer,
            configuration: configuration
    ...
    func hostCommitBoundaryInvalidatesShortcutState() {
        ...
        #expect(!HangyeolInputController.routeHostCommitComposition(
            in: session,
            sender: staleClient
        ))
    ...
    func deferredReturnReplayInvalidatesMatchingSession() {
        ...
        #expect(!HangyeolInputController.routeDeferredHostKey(
            in: session,
            client: staleClient,
    ...
    func hostPassedFieldBoundariesInvalidateContext() async {
        ...
        for boundary in fieldBoundaryKeys {
            ...
            #expect(!HangyeolInputController.routeSecureKeyDown(
                in: session,
                keyCode: 15
            ))
            #expect(!session.finalize(reason: .appDeactivate))
    ...
    @Test("Secure cleanup rejects marked text no longer owned by Hangyeol")
    ...
    func sameClientReactivationRejectsSameLengthUnownedMarkedText(
        ...
        // happens to have the same UTF-16 length as Hangyeol's previous preedit.
        client.markedText = "나"
        client.markedRangeValue = NSRange(location: 0, length: 1)
    ...
    func secureThenUnrelatedNormalReactivationDoesNotWrite() {
        ...
        #expect(!HangyeolInputController.routeSecureKeyDown(in: session, keyCode: 15))
        #expect(!composer.hasActiveComposition)
        #expect(client.insertCalls.count == insertCount)
        #expect(client.markCalls.count == markCount)
    ...
    func blinkActivationHandoffIsWriteFree() {
        ...
        #expect(HangyeolInputController.retireSessionForControllerHandoff(
            retirement,
    ...
    func lateInputBoundaryHandoffIsWriteFree() {
        ...
        #expect(HangyeolInputController.retireSessionForLateInputBoundaryHandoff(
            retirement,
            currentSession: { oldSession }
        ) {
            cleanupCount += 1
