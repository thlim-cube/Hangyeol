...
/// ```
@objc(HangyeolInputController)
public class HangyeolInputController: IMKInputController, @unchecked Sendable {
    ...
    private static let activeControllerRegistry = ActiveOwnerHandoffRegistry<HangyeolInputController>()
    public static var sharedController: HangyeolInputController? {
    ...
    public func performHangyeolModeTransition(source: InputModeCoordinator.ToggleSource) {
        performHangyeolModeTransition(source: source, trace: .begin(source: source))
    }

    public func performHangyeolModeTransition(
        source: InputModeCoordinator.ToggleSource,
        trace: ToggleLatencyTrace
    ) {
        guard applyPendingHangyeolModeTransition(source: source, trace: trace) else {
    ...
    @discardableResult
    func applyPendingHangyeolModeTransition(
        source: InputModeCoordinator.ToggleSource,
        trace: ToggleLatencyTrace
    ) -> Bool {
        guard Self.sharedController === self,
    ...
    /// Hangyeol registers one IMK mode and owns Korean/English state internally.
    ...
    /// to update marked text even though Hangyeol supplies no `composedString`.
    ...
    override public func handle(_ event: NSEvent!, client sender: Any!) -> Bool {
        ...
        // A previous Secure Input pass-through may have left Hangyeol-owned marked
