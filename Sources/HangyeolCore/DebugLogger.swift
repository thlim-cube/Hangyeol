...
public final class DebugLogger: @unchecked Sendable {
    ...
    private static let osLog = OSLog(subsystem: ProductIdentity.bundleID, category: "Debug")
    
    /// Serial queue for thread-safe file operations
    private static let logQueue = DispatchQueue(
        label: "\(ProductIdentity.preferencePrefix).logger",
        qos: .utility
    )
    ...
    private static func writeToFile(data: Data) throws {
        let url = URL(fileURLWithPath: HangyeolConfig.logPath)
        ...
        if !FileManager.default.fileExists(atPath: HangyeolConfig.logPath) {
            FileManager.default.createFile(atPath: HangyeolConfig.logPath, contents: nil)
            cachedHandle = nil // Invalidate cache
        }
        
        // Get or create file handle
        if cachedHandle == nil {
            cachedHandle = FileHandle(forWritingAtPath: HangyeolConfig.logPath)
        }
