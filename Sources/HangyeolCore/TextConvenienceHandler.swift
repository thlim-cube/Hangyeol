...
public final class TextConvenienceHandler: @unchecked Sendable {
    ...
    public func handleDoubleSpacePeriod(buffer: inout String, delegate: HangulComposerDelegate, checkHangul: Bool = false) -> DoubleSpaceResult {
        ...
        let isDoubleTap = (now - lastSpaceTime) < HangyeolConfig.doubleSpaceThreshold
        lastSpaceTime = now
    ...
    /// Handles macOS-like text conveniences in Hangyeol English mode.
    ///
    ...
    /// not do it for Hangyeol's internal English mode. It is disabled by default
