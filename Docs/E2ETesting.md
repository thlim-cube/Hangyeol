...
## 3.1.8 현재 세션·다음 로그인 설치 전환 (2026-09-23)

사용자 요구: 등록 구조가 같은 업데이트를 설치하면 재로그인 전에도 새 코드로
입력하고, 재로그인 후에는 `/Library/Input Methods/Hangyeol.app`의 새 코드로
계속 입력한다. 설치 직후 키보드를 삭제·재추가하지 않아도 입력 소스 목록과
`한결 설정…`·`한결 정보`가 보여야 한다. 초기 설치와 등록 구조 변경은 기존
TIS 복구 절차를 사용한다.

일반 업데이트의 설치 전 스크립트는 패키지에 함께 든 동일 서명의 앱을
`/private/tmp/hangyeol-session.XXXXXX/Hangyeol.app`에 복사·검증한다.
기존 한결만 종료하고 임시 앱을 시작한 뒤, 실행 파일 경로가 임시 앱 하나이고
한결 parent·mode가 활성 상태이며 설치 전 선택 상태가 유지되는지 확인한다.
실패하면 정식 앱 교체 전에 이전 앱을 다시 열고 설치를 중단한다. 설치 후에는
임시 런타임을 재시작하지 않고, 새 패키지 receipt를 확인하는 복구 요청을
다음 로그인용 정식 경로에 남긴다. 임시 앱은 그 요청을 소비하지 않는다.

수용 검증은 한 번의 실제 설치 전후에 네 경계를 구분한다: (1) 설치 전후
입력 소스 목록과 한결·설정·정보 메뉴, (2) 설치 직후 임시 실행 파일의
...
실제 설치에서 한결 메뉴가 사라졌다. `TISRegisterInputSource`,
`TISEnableInputSource`, ABC→한결 선택과 정식 앱 실행은 성공하거나
`selected`·`ready`를 반환했지만 메뉴는 돌아오지 않았다. 사용자가 시스템
설정에서 한결을 제거·재추가한 뒤 한결·설정·정보 메뉴가 복구됐다. 따라서
TIS 상태나 프로세스 실행만으로 설치 성공을 판정하지 않는다.

### 3.1.10 일반 업데이트의 메뉴 유지 검증

사용자 우선순위는 설치 직후 한결이 사라지지 않는 것이다. 등록 구조가 같은
일반 업데이트에서는 설치 전 현재 IMK 프로세스를 종료하거나 임시 앱을
실행하지 않고, 설치 후에도 현재 세션의 프로세스를 재시작하거나 TIS 소스를
전환하지 않는다. 새 파일은 정식 경로에 설치하고 다음 로그인 복구 요청만
예약한다. 이번 단계에서 현재 세션은 기존 실행 코드로 계속 입력하며, 새
코드의 즉시 적용은 아래 수용 검증 후 별도로 진행한다.

설치 전후의 실제 입력 메뉴에서 한결·한결 설정·한결 정보가 계속 보이고,
한영 전환과 한글 입력이 동작해야 한다. 설치 전후 실행 중인 한결 PID가
같은지 확인하고, 패키지 receipt와 디스크 정식 앱은 3.1.10/126 및 유효한
Apple Development 서명이어야 한다. 재로그인 후에는 정식 경로의
3.1.10/126 프로세스가 실행되고 같은 메뉴·입력이 동작해야 한다. 사용자가
입력 소스를 삭제·재추가하지 않은 상태에서 이 경계를 확인한다. 한결이
사라지면 이 접근은 실패로 기록하고 즉시 적용 단계로 넘어가지 않는다.

실제 3.1.10 설치에서는 11:32부터 실행 중이던 한결 PID 66132가 설치 후에도
남아 있었고, 13:05:42 패키지 receipt와 디스크 앱은 3.1.10으로 갱신됐다.
그러나 입력 메뉴의 한결·설정·정보가 사라지고, 설치된 앱의 상태 조회는
`candidates=false enabled=false ready=false`였다. PackageKit 로그에는
`Contents` payload 적용과 설치 완료 뒤 구성요소 갱신 통지가 남았다.
따라서 프로세스 종료를 없애는 방식은 수용 조건에 실패했다.

### 3.1.11 재부팅 전용 단계적 업데이트

3.1.10에서 프로세스를 살려도 정식 번들 갱신 직후 메뉴가 사라진 것을
확인했다. 3.1.11 로컬 패키지는 설치 중
`/Library/Input Methods/Hangyeol.app`에 파일을 쓰지 않는다. 새 버전은
`/Library/Application Support/Hangyeol/Updater/pending-app.tar.gz`에
보관하고, 루트 LaunchDaemon이 다음 재부팅에 로그인 세션과 한결 프로세스가
없는 상태에서만 서명·체크섬을 확인해 정식 앱과 교체한다. 교체에 실패하면
이전 앱을 복원한다. 입력 소스 복구 요청은 새 버전으로 다음 로그인에 실행된다.

실제 수용 조건은 입력 소스 삭제·재추가 없이 (1) 설치 전후 같은 한결
프로세스와 한결·설정·정보 메뉴 및 한글 입력 유지, (2) 설치 직후 정식 앱은
이전 버전이고 별도 보관 앱은 3.1.11/127이며 유효한 Apple Development
서명, (3) 재부팅 후 정식 앱과 실행 프로세스가 3.1.11/127이고 메뉴·입력이
정상인 것이다. 설치가 실패하거나 메뉴가 사라지면 원인을 확인할 때까지
즉시 적용 단계로 넘어가지 않는다.

실제 3.1.11 설치 직후에는 한결 메뉴가 사라졌고 재부팅 후 복구됐다.
설치 패키지는 `/Library/Input Methods/Hangyeol.app`을 쓰지 않았으므로,
정식 번들 교체만으로 설명할 수 없다. 설치 로그의 15:12:04–06에는
`postinstall`이 임시 위치의 서명 앱 실행 파일을 호출하고 사용자 복구
LaunchAgent를 갱신한 기록이 있으며, 그 뒤 `TextInputMenuAgent`가 입력
메뉴를 다시 구성했다. 어느 동작이 한결 소실을 일으켰는지는 아직
분리되지 않았다. 15:25 재부팅에서는 시스템 LaunchDaemon이 로그인 뒤
실행되어 “desktop session is active”로 교체를 연기했고, 정식 앱은
3.1.10/126으로 남았다. TIS 상태는 정상으로 복구됐다.

### 3.1.12 로그인 세션과 설치 스크립트 분리

로컬 패키지에서 `preinstall`·`postinstall`과 임시 한결 앱 실행을 모두
제거한다. 설치 중 PackageKit이 만지는 경로는 보관 압축 파일과 시스템
LaunchDaemon뿐이다. 시스템 작업은 로그인 중에는 정식 번들을 변경하지
않고 로그아웃을 기다린다. 자동 로그인으로 부팅 시점을 놓쳐도 다음
로그아웃까지 대기한다.

수용 검증은 (1) 패키지에 Scripts 항목과 입력기 경로 payload가 없는지,
(2) 설치 직후 한결·설정·정보 메뉴, 한글 입력, 기존 실행 PID와 정식
3.1.10 앱이 유지되는지, (3) 시스템 작업을 시작해 로그인 중에는 대기하고
로그아웃 뒤 정식 앱이 3.1.12/128로 바뀌는지, (4) 다시 로그인한 후 메뉴와
한글 입력이 정상인지 확인한다. 사용자 재등록으로 복구한 결과를 설치
성공으로 취급하지 않는다.

2026-09-23 실제 설치에서 staged-update receipt는 3.1.12로 갱신됐고,
기존 한결 PID 3328과 정식 앱 3.1.10/126이 유지됐다. 사용자는 설치 직후
메뉴가 사라지지 않았다고 확인했다. TIS 상태도
`candidates=true enabled=true ready=true`였다. 패키지에는 설치 스크립트와
입력기 경로 payload가 없으며, 별도 보관 앱 압축 파일의 체크섬이 일치했다.

이 호스트에는 3.1.11에서 등록된 시스템 작업이 남아 있어
`launchctl kickstart -p system/com.thlim.hangyeol.apply-staged-update`로
새 스크립트를 시작했다. PID 12473은 로그인 중 교체하지 않고
`waiting for logout and input-method exit`를 기록했다. 사용자가 작업을
마친 뒤 로그아웃·재로그인하자 `staged app installed outside the login
session`이 기록됐고, 정식 앱은 3.1.12/128 및 유효한 Apple Development
서명으로 확인됐다. 보관 압축 파일과 체크섬은 적용 후 제거됐다.
16:24:20에 정식 경로에서 시작한 PID 19805와 정상 TIS 상태를 확인했고,
사용자는 수동 삭제·재추가 없이 설정·정보 메뉴, 한글 입력과 한영 전환이
모두 정상이라고 확인했다.

이 결과는 기존 시스템 작업을 수동 시작한 설치·로그아웃 경로의 통과
증거다. 새로 설치한 LaunchDaemon의 재부팅 자동 시작까지 검증한 것은
아니며, 현재 로그인 세션에 새 버전을 즉시 적용하는 기능도 포함하지 않는다.

### 3.1.13 설치 직후 임시 적용

2026-09-23 사용자는 재부팅 자동 적용을 된 것으로 두고 설치 직후 임시
적용을 진행하도록 요청했다. 재부팅 자동 시작 자체는 이번 검증의 선행
가정이며 새로 관찰한 통과 결과로 기록하지 않는다.

정식 앱을 그대로 둔 사전 실험에서 임시 앱의 실행 파일을 직접 시작하면
한결 선택 시 정식 경로의 프로세스가 함께 실행됐다. ABC로 잠시 이동한
상태에서도 재선택하면 같은 중복이 발생했다. 시스템 앱 실행 경로로
임시 앱을 시작한 뒤에는 재선택 후에도 임시 경로의 프로세스 하나만
확인됐다. 이는 실행 경로 증거이며 메뉴·실제 입력 증거와 구분한다.

| 요구 동작 | 검증 경계 |
| --- | --- |
| 설치 중 정식 번들을 변경하지 않고 메뉴와 입력을 유지한다 | 패키지 payload 확인과 설치 직후 호스트 확인 |
| 현재 세션에서는 서명된 새 버전 하나만 입력을 처리한다 | 도우미가 등록 호환성·기존 앱 서명 요건 확인, 입력 소스 복귀 전후 프로세스 경로 검사, 새 버전 정보와 실제 타이핑 |
| 후보 검증 실패 시 기존 입력기를 종료하지 않는다 | `SessionRuntimeActivationTests.invalidCandidateLeavesCurrentInputUntouched` |
| 전환 중 오류가 나면 정식 앱을 복구하고 성공으로 숨기지 않는다 | 단계별 실패·복구 실패 단위 테스트와 도우미 실행 결과 |
| 사용자가 별도로 선택한 입력 소스를 덮어쓰지 않는다 | `installerRestoresOnlyTheFallbackItSelected` |
| 임시 앱이 다음 로그인이나 다른 사용자 세션에서 계속 실행되지 않는다 | `SessionRuntimeLease` 단위 검사, 재로그인 후 정식 실행 경로 |

3.1.12의 설치 스크립트 없음 조건은 이번 사용자 요구에 따라 제한된
`postinstall_session` 실행으로 대체한다. 스크립트는 임시 앱을 CLI로
실행하거나 정식 입력기를 덮어쓰지 않고 독립 도우미만 호출한다.
현재 세션의 임시 앱은 사용자·보안 세션 번호가 기록된 파일로 범위를
제한하며, 새 로그인에서 호출되면 정식 앱으로 넘긴다. 기존 시스템
갱신 작업은 로그인 중 기다리고 로그아웃 뒤 정식 앱을 교체한다.


2026-09-23 17:05 실제 PackageKit 설치 로그는 `session-activation=applied`와
`/private/tmp/hangyeol-session.PzPhII/Hangyeol.app` 실행을 기록했다.
설치 영수증은 3.1.13이며 정식 경로는 3.1.12/128로 유지됐다. 현재 입력기는
임시 경로의 PID 32535 하나이며 버전 3.1.13/129, Apple Development 팀
`9FRJXJNGZK`, CDHash `9d30ba55d3391d4f02ca7f1359264229f0cb851b`로
패키지 앱과 일치했다. 시스템 교체 작업 PID 32544는 로그아웃을 기다린다.

사용자는 설치 후 한결이 존재하는 것을 봤다고 확인했다. 현재 원격이라
직접 조작할 수 없다고 했으므로 설정·정보 메뉴와 수동 한영 전환의 관찰은
완료로 기록하지 않는다. 설치본이 시작한 임시 앱을 대상으로 프로젝트
`HangyeolE2E --app /private/tmp/hangyeol-session.PzPhII/Hangyeol.app
--scenario '두벌식 조합'`을 실행해 TextEdit 조합, Chrome input/contenteditable
조합, 기존 영문 모드 복원 3/3을 통과했다. 결과는
`.build/session-package-e2e.log`에 기록했다. 자동 검사 시작 시 한결의 내부
영문 모드였으며 종료 시 이를 복원했다.

최종 코드의 `swift test`는 62개 suite, 598개 테스트를 통과했고 셸 문법,
`git diff --check`, 앱·도우미 서명, payload에 정식 입력기 경로가 없는 조건과
제한된 postinstall 내용 검사를 통과했다. 이전 버전용 활성화 요청은 새
버전에서 별도 IMK를 시작하지 않도록 제외했다.

별도 사전 실험에서는 세션 번호가 만료된 임시 앱을 실행해 정식 앱으로
복귀하는 것을 확인했다. 반복 실행 시 중복 정식 프로세스가 생긴 결함은
복귀 실행을 파일 잠금으로 직렬화하고 이미 실행 중인 정식 앱을 확인해
수정했다. 수정 후 최초·반복 실행 모두 정식 프로세스 하나로 유지됐으며,
...
### 3.1.15 연결 이름 변경 시 현재 입력기 보존

2026-09-25 사용자는 실제 3.1.14 설치 후 한결 항목은 있지만 한영 전환과
설정·정보 메뉴가 동작하지 않는다고 보고했다. 설치 로그는 05:21에
`session-activation=applied`였고, 정식 앱은 3.1.12/128의
`Hangyeol_InputString`, 임시 PID 64260은 3.1.14/130의
`com.thlim.inputmethod.Hangyeol_Connection`이었다. imklaunchagent의
연결 이름 거부와 `requestIMKXPCEndpointInvalid`도 반복됐다. 앞 절의
수동 임시 실행 성공은 실제 PackageKit 설치의 성공을 보장하지 못했다.

사용자가 우선한 설치 중 입력 보존 요구에 따라, 연결 이름 변경은 예외 없이
정식 경로를 교체하는 로그아웃 시점까지 미룬다. 기존의 알려진 이름 변경
허용 테스트는 실제 실패가 확인되어 거부 기대값으로 수정했다. 이름이 같은
정상 업데이트는 기존 즉시 적용 경로를 유지하며, 빈 이름·누락·양방향 변경은
거부한다. 거부 시 입력 소스 전환, 프로세스 종료·실행을 시작하지 않아야 한다.
타이핑 처리나 지연, 기존 서명·등록 정보 검증은 변경하지 않는다.

수정 전 회귀 검사는 이름 변경과 빈 이름 허용에서 실패했고 수정 후 통과했다
(`.build/connection-guard-before.log`, `.build/connection-guard-after.log`).
현재 세션 복구에서는 정식 앱을 실행하고 선택 가능한 입력 모드를 새로
조회해 선택했다. 05:31 TextEdit·Chrome 실제 키 조합 2/2가 통과했다
(`.build/install-recovery-host.log`). 이후 05:33 시스템이 재부팅됐고 진단에는
`SOCD report detected: (AP watchdog expired)`가 기록됐다. 해당 기록만으로
입력기와 재부팅의 인과관계를 판단하지 않는다. 사용자의 설정·정보 메뉴 정상
확인은 재부팅 후이므로 무재부팅 메뉴 복구 증거로 취급하지 않는다. 재부팅 후
정식 앱은 여전히 3.1.12이고, 지연 업데이트는 로그인 세션 종료를 기다리고 있다.

수정된 도우미에 유효한 현재 세션 번호와 실제 서명된 3.1.14 후보를 전달하자
연결 이름 차이를 이유로 `session-activation=deferred`를 반환했다. 호출 전후
실행 PID 목록, 한결 선택 상태, 정식 실행 파일 SHA-256이 모두 동일했다
...
### 3.1.17 실제 설치·재로그인과 TextEdit 첫 확인

06:06:00 설치 로그는 `session update applied`를 기록했고 임시 경로
3.1.17/133 PID 하나가 패키지 앱과 같은 CDHash
`f7aafbc7ede511812b78056db7ecebb148e10678`로 실행됐다. TextEdit·Chrome
두벌식 조합 2/2가 통과했다(`.build/install-3.1.17-e2e.log`). 재로그인 후
정식 번들은 같은 CDHash의 3.1.17/133으로 교체됐고 교체 작업은 종료 코드 0,
대기 파일 정리, 정식 경로 PID 하나를 확인했다.

교체 직후 첫 검사에서는 TextEdit만 `gksrmf\n`으로 실패했고 곧바로 다시 실행한
검사는 3/3 통과했다(`.build/relogin-3.1.17-e2e.log`,
`.build/relogin-3.1.17-e2e-rerun.log`). 새 IMK 프로세스의 기본 모드는 한글이고
모드는 저장되지 않으므로, 한 번의 `rk` 확인 결과를 영문 모드로 보고 전환키를
누른 러너 판단이 한글 모드를 영문으로 바꾼 것으로 추정한다. TextEdit 준비는
Chrome과 같이 `rk`가 나오면 한 번 더 확인하고, 두 확인 결과를 출력한다.

수정한 러너는 영문 시작 3회와 한글 시작 5회를 통과했다. 도우미로 새 프로세스를
띄운 직후 3회와 교체 없는 재로그인 직후 1회에서 모두 `first=가`였다
(`.build/e2e-fresh-launch-*.log`, `.build/e2e-after-relogin-probe.log`). 따라서
첫 키가 한결을 거치지 않는 현상은 재현하지 못했다. 실패한 검사와 달리 이
재로그인에는 정식 번들 교체가 없었으므로, 교체 직후 첫 실행에만 생기는지는
다음 업데이트의 재로그인 직후 첫 검사에서 확인한다. 수정 직후 한 번 관찰한
TextEdit 빈 값(`actual=""`) 실패는 어느 확인에서 났는지 출력이 없어 원인을
판단하지 않는다.
