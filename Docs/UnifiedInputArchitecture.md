# Hangyeol 통합 입력 아키텍처 (Unified Input Architecture)
...
| Caps Lock = macOS 소유 | 아님 (Hangyeol이 가로챔) | 맞음 |

**근본 충돌:** 실제 ABC source가 선택되는 순간 Hangyeol은 현재 IMK 입력 세션 소유권을 잃는다.
그래서 "진짜 ABC + 무지연 내부 전환"은 양립 불가능하다. 전환 race(2.7.2의 씹힘/불일치)는
*실제 입력 소스 선택*과 *Hangyeol 내부 composer mode*가 서로 다른 비동기 시스템이라는 데서 온다.

**결합안:** custom 전환키 경로에서는 Hangyeol 단일 입력 소스가 활성 IMK 세션을 유지하고, process-global
...
Caps Lock 기반 전환은 예외로 macOS가 실제 TIS source를 소유하고, 이때 Hangyeol custom toggle은 비활성화한다.
macOS 소유권 활성화 또는 ABC→Hangyeol 재선택 경계는 pending으로 기록하고, 다음 비보안 Hangyeol
...
CGEventTap / IOKit  ──(키 감지만)──►  InputModeCoordinator   (정책: Caps Lock·controller 유무)
                                            │ requestToggle
                                            ▼
                              HangyeolInputController          (IMK 세션 imperative edge)
                                  │ performHangyeolModeTransition
        ┌─────────────────────────┼──────────────────────────┐
        ▼                         ▼                          ▼
...
- [StatusBarManager](../Sources/HangyeolCore/StatusBarManager.swift)는 과거 공개 API의 source compatibility를 위한 no-op shell이며 별도 메뉴 막대 아이콘을 표시하지 않는다.
...
**트레이드오프(수용):** 영어 모드일 때도 macOS 메뉴바의 입력 소스 아이콘은 Hangyeol(한글)로 남는다.
Hangyeol은 별도 `한`/`A` 상태 아이콘을 추가하지 않는다. 실제 mode는 `InputModeStore`, pending 소유권 정합화는 `InputModeOwnershipTracker`, 감시 backend 수명은 `ToggleMonitorStatusStore`가 각각 보관한다.
...
| IMK 세션 edge(commit·override·layout) | `HangyeolInputController` | imperative 경계 |
...
2. 프로덕션에서 `composer.inputMode`를 바꾸는 writer는 `HangyeolInputController`뿐이다.
   사용자 토글은 `performHangyeolModeTransition`, macOS 소유권 변경/ABC→Hangyeol 재선택은
...
5. 영어 편의 fallback이 꺼진 기본 상태에서 Hangyeol은 printable key를 consume하지 않는다(`return false`).
...
[HangulComposer.handle()](../Sources/HangyeolCore/HangulComposer.swift)의 영어 분기는 조합 정리 후
기본적으로 `return false`를 수행한다. 영어의 더블스페이스 마침표 등 텍스트 편의는 macOS와 host 앱이
소유한다(2.7 결정과 일치). 이로써 기본 경로에서 "Hangyeol 영어 버퍼 ↔ 실제 커서" desync와 host 치환
중복 가능성을 제거한다.
...
`RightCommandSuppressor/IOKit → InputModeCoordinator → HangyeolInputController → composer`로 일원화해,
...
또는 ABC→Hangyeol 재선택은 실제 입력 소스 경계이므로 예외다. 이 경계는 coordinator가 pending으로만
...
```bash
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build -c debug --product Hangyeol
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift run -c debug HangyeolVerify
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift build -c release --product Hangyeol
```
...
- Caps Lock 전환 on — custom toggle 비활성, macOS만 ABC↔Hangyeol 전환
- Caps Lock 전환 on, ABC→Hangyeol 복귀 — 첫 비보안 keyDown 전에 내부 mode가 한국어로 정합화됨
- 동일 Hangyeol source에서 탭·앱·필드 이동 — 마지막 한/영 mode 유지
...
- 일반 앱 시작·typing hot path에서는 Hangyeol 자신을 `TISEnableInputSource` 하지 않는다. 설치 때만 사용자별 1회성 복구 작업이 action과 별도-process verifier를 짝지어 등록·활성화를 수행하며, 실패한 generation은 다음 로그인까지 보존한다.
